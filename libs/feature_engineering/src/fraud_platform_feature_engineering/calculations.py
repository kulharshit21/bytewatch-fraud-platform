...
from fraud_platform_feature_engineering.models import MODEL_FEATURE_FIELDS, FeatureContext
...
def compute_feature_values(event: TransactionEvent, context: FeatureContext) -> dict[str, float]:
    ...
    count_1h = sum(
        1 for ts in timestamps if (event_ts - ts.astimezone(UTC)).total_seconds() <= 3600
    )
    ...
    country_mismatch = float(
        bool(
            ...
            and context.last_country
            and context.last_country != event.country
        )
    )
