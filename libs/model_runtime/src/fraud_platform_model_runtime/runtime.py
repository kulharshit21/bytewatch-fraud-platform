...
def build_reason_codes(features: dict[str, float], rule_hits: list[RuleHit]) -> list[ReasonCode]:
    ...
    heuristics = [
        ("velocity_spike", "Recent transaction velocity is elevated.", features.get("tx_count_5m", 0.0) >= 4),
        ("geo_jump", "Transaction location changed sharply.", features.get("geo_distance_from_last_tx_km", 0.0) >= 500.0),
        ("new_device", "Transaction originated from a new device.", features.get("device_new_for_account", 0.0) >= 1.0),
        ("amount_spike", "Amount is far above recent average.", features.get("amount_vs_recent_avg_ratio", 0.0) >= 3.0),
        ("high_risk_merchant", "Merchant is flagged as high risk.", features.get("high_risk_merchant_flag", 0.0) >= 1.0),
        ("auth_failures", "Recent authentication failures were observed.", features.get("failed_auth_count_recent", 0.0) >= 1.0),
    ]
...
def combine_model_and_rules(
    ...
    if any(hit.severity == "critical" for hit in rule_hits) or final_score >= model_metadata.block_threshold:
        return final_score, FraudDecision.BLOCK
    if final_score >= model_metadata.review_threshold or any(hit.severity == "high" for hit in rule_hits):
...
class ModelRuntime:
    ...
    def reload(self) -> LoadedModel:
        ...
        model_uri = f"models:/{self.settings.mlflow_model_name}@{self.settings.mlflow_champion_alias}"
    ...
    def score(self, features: dict[str, float], rule_hits: list[RuleHit]) -> tuple[float, float, FraudDecision, ModelMetadata, list[ReasonCode]]:
        loaded = self.ensure_loaded()
        frame = pd.DataFrame([{name: float(features.get(name, 0.0)) for name in loaded.feature_names}])
        ...
        MODEL_PREDICTION_COUNTER.labels(decision=decision, model_alias=loaded.metadata.model_alias).inc()
